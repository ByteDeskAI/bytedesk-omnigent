...
class _JiraClient(HttpToolClient):
    ...
    def _resolve_credentials(self) -> None:
        ...
        if self._connection_id:
            ...
            creds = resolve_atlassian_credentials(self._connection_id, service="jira")
    ...
    def _require_configured(self) -> None:
        ...
        if self._headers_override:
            if not self._base_url:
                raise JiraNotConfiguredError(_SECRET_BASE_URL)
            return
...
class BytedeskJiraTool(Tool):
    ...
    @classmethod
    def from_config(cls, config: dict[str, str] | None = None) -> BytedeskJiraTool:
        config = config if isinstance(config, dict) else {}
        connection_id = config.get("connection_id") or None
        return cls(client=_JiraClient(connection_id=connection_id))
