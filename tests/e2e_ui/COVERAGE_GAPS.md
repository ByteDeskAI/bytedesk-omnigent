...
| 🧪 | **Model selector / cost-routing control** in composer | `components/ai-elements/model-selector.tsx`, `components/CostRoutingControl.tsx` | Cost-routing renders only for top-level `polly` sessions (`costRoutingEligible`), which the e2e `hello_world` fixture does not create, and the model picker renders only for `claude-code-native-ui` wrapper sessions (would need a real Claude-native boot). The logic is unit-covered by `CostRoutingControl.test.tsx` and the `/model` plus cost-router composer cases in `ChatPage.composer.test.tsx`. |
