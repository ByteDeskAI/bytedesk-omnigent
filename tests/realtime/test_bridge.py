...
def test_emit_roster_publishes_when_tenant_set(monkeypatch):
    ...
    assert calls == [
        ("office:agents:tenant-abc", {"type": "roster.changed", "action": "created", "agentId": "ag_1"})
    ]
...
def test_wrap_create_calls_orig_then_emits(monkeypatch):
    ...
    assert calls == [("office:agents:t", {"type": "roster.changed", "action": "created", "agentId": "ag_9"})]


def test_wrap_update_skips_emit_when_no_row(monkeypatch):
    monkeypatch.setenv("BYTEDESK_REALTIME_TENANT_ID", "t")
    calls = _capture(monkeypatch)
    assert bridge.wrap_update(lambda self, agent_id, bundle_location: None)(object(), "ag_x", "loc") is None
    assert calls == []
...
def test_wrap_update_emits_when_row_updated(monkeypatch):
    ...
    assert calls == [("office:agents:t", {"type": "roster.changed", "action": "updated", "agentId": "ag_u"})]
...
def test_wrap_delete_emits_only_when_existed(monkeypatch):
    ...
    assert calls == [("office:agents:t", {"type": "roster.changed", "action": "deleted", "agentId": "ag_d"})]
