...
| **Flag sprawl (BDP-2511).** Several strangler flags are still live (`OMNIGENT_STORE_LIFECYCLE_HOOKS`, plus `_SPEC_SOURCE`, `_COORDINATION_BACKPLANE`, `_AGENT_MEMORY`, `_MEMORY_EMBEDDER`, `_BACKOFF_POLICY`, `_SCHEMA_VALIDATOR`, `_REMOTE_FUNCTION`, and `OMNIGENT_DISABLED_EXTENSIONS`). The service-registry, DI-container, tool-execution-context, tool-dispatcher-registry, and lifespan-phase gates have been retired. | core | Treat each remaining flag as a strangler with an explicit retirement: dual-run → parity → flip-default → delete-twin → remove-flag. Track the rest under BDP-2511 so flags don't outlive their cutover. |
