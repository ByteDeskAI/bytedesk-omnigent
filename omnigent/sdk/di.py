"""SDK — the dependency-injection container (BDP-2508).
...
It gives an extension:
  ...
  * **Method injection** — ``call(fn)`` resolves a function's annotated params,
    so a ``@tool`` / ``@harness`` factory can declare
    ``def build(self, store: ArtifactStore)`` and the container supplies
    ``store``.
  * **Interface registration** — register a concrete class under a Protocol/ABC
    key (``register_type(Impl, key=Interface)``) so an extension depends on a
    *capability* without knowing the concrete class (Dependency Inversion).
  * **Child scopes** — ``create_scope()`` inherits singletons but isolates
    scoped/transient instances.
  * **Cycle detection** — a resolution cycle raises :class:`DIResolutionError`
...
logger = logging.getLogger("omnigent.sdk.di")

T = TypeVar("T")


class Lifetime(Enum):
    """Service lifetime — how often the container builds a fresh instance."""
...
class Container:
    ...
    A real microkernel resolves collaborators for its extensions instead of
    letting them ``import`` each other into a tangle. Registering by a
    Protocol/ABC and resolving by it is how an extension depends on a
    *capability* without knowing the concrete class.
    """
    ...
    def try_resolve(self, key: Any, default: Any = None) -> Any:
        """Resolve *key*, returning *default* if it is unregistered."""
        try:
    ...
    def call(self, fn: Callable[..., T]) -> T:
        ...
        This is how ``@tool`` / ``@harness`` factories get their collaborators
        injected.
        """
    ...
    def _build_kwargs(self, fn: Callable, *, skip_self: bool = False) -> dict[str, Any]:
        try:
            ...
            return {}
        try:
            # Resolves string annotations against the fn's module globals.
            hints = get_type_hints(fn)
        except Exception:  # noqa: BLE001
            # Fall back to raw annotations — handles function-local class types
            # that get_type_hints cannot resolve. Non-stringized annotations are
            # already the type object; stringized ones are dropped (unresolvable).
            raw = getattr(fn, "__annotations__", {})
            hints = {k: v for k, v in raw.items() if not isinstance(v, str)}
        kwargs: dict[str, Any] = {}
...
__all__ = ["Container", "Lifetime", "DIResolutionError"]
