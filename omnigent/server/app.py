...
_GROK_NATIVE_AGENT_NAME = GROK_NATIVE_CODING_AGENT.agent_name
...
def _build_grok_native_bundle() -> bytes:
    """
    Build a gzipped tarball of the grok-native-ui agent spec.
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        spec_path = _materialize_grok_agent_spec(Path(tmpdir), model=None)
        bundle_dir = materialize_bundle(spec_path, Path(tmpdir) / "bundle")
        return _tar_gz_dir(bundle_dir)
...
def _ensure_default_grok_agent(
    ...
    """
    Register or refresh the grok-native-ui agent (ByteDesk).

    Called during server lifespan startup so the Web UI offers Grok as a
    built-in native coding agent alongside Claude / Codex / Pi. Content-aware
    via :func:`_ensure_builtin_agent`: a changed spec refreshes the row in
    place rather than being ignored.
    ...
    _ensure_builtin_agent(
        ...
        artifact_store,
        agent_cache,
        name=_GROK_NATIVE_AGENT_NAME,
        bundle_bytes=_build_grok_native_bundle(),
    )
...
def create_app(
    ...
    # AP-server-side MCP proxy pool. Manages connections to agents' external
    # MCP servers on behalf of ``POST /v1/sessions/{id}/mcp``. Created here
    # before lifespan and route registration so the same object is closed by
    # lifespan and held by the router closure.
